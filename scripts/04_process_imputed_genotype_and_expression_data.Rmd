...
library(duckdb)
#con <- dbConnect(duckdb::duckdb(), dbdir = ":memory:")  # Use in-memory DB (or specify file path for persistence)
# Load file into DuckDB as a virtual table without loading it fully into memory
#large_data <- dbGetQuery(con, "CREATE OR REPLACE TABLE large_table AS SELECT * FROM read_csv_auto('../snp_array/TOPMED_imput/transposed_data.txt')")

# Read the raw genotype file
genotype_raw <- fread("../snp_array/TOPMED_imput/gwas_filtered_data.clean_genotype.raw")
...
write.table(genotype_data, file = "../snp_array/gwas_filtered_genotype_data.txt", sep = " ", quote = FALSE, col.names = NA)
```
