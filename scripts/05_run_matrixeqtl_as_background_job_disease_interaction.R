...
# Last but not least is the model testing for the significance of the interaction between the genotype and the last covariate. This model can be used to test for equality of effect sizes (as in the standard model) between two groups of samples (case vs. control). In this case the last covariate must be the dummy variable indicating the sample groups.
...
gene$LoadFile("../RNA_counts/n579_TPM_counts.txt") # or combat_adjusted
...
cvrt$LoadFile("../snp_array/covariates_and_phenotype_files/covariates_with_disease_status_batch.txt")
...
me <- Matrix_eQTL_main(
  ...
  gene = gene,
  cvrt = cvrt,
  output_file_name =  file.path("../MatrixEQTL/n579_all_eQTL_disease_interaction_batch"),
  pvOutputThreshold = pvOutputThreshold, 
  useModel = modelLINEAR_CROSS, ## test using linear models 
  verbose = TRUE,
  pvalue.hist = TRUE,
...
plot(me)
saveToPDF("../MatrixEQTL/qqplot_pvalues_eqtl_disease_interation_batch.pdf", width = 7, height = 6)
# Save as RDS 
saveRDS(me, "../MatrixEQTL/n579_eQTL_disease_interaction_batch.Rds")
