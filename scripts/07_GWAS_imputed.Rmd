...
```{r read_gwas_results}
# Read in GWAS file 
gwas <- fread("../snp_array/cingLBD_association_imputed.assoc.linear")
```

### ggmanh
```{r}
gwas$CHR[is.na(gwas$CHR)] <- "X"
chr_labels <- c(1:22, "X")
gwas$CHR <- factor(gwas$CHR, levels = chr_labels)
chr_colors <- c("navyblue", "skyblue2")

gwas$label <-  ifelse(gwas$P < 0.00001, gwas$SNP, NA)

subset(gwas, SNP == "rs3857059")
g <- manhattan_plot(x = gwas, pval.colname = "P", chr.colname = "CHR", pos.colname = "BP", plot.title = "GWAS Cingulate Lewy body count", y.label = "-log10(p-value)", chr.col = chr_colors, label.colname = "label", label.font.size = 2.75, highlight.colname = "label", highlight.col = c("black")) 
g
...
# Read in GWAS file 
getwd()
gwas <- fread("snp_array/Thal_association_imputed.assoc.linear")
...
g <- manhattan_plot(x = gwas, pval.colname = "P", chr.colname = "CHR", pos.colname = "BP", plot.title = "GWAS Thal amyloid phase", y.label = "-log10(p-value)", chr.col = chr_colors, label.colname = "label", label.font.size = 2.75, highlight.colname = "label", highlight.col = c("black")) 
g
